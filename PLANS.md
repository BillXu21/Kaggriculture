...
Current active packet (issue #9 Stage A, 2026-08-24): the RL rollout/self-play/trajectory harness `rl_manager` is implemented and locally validated — batched own-only-E JAX policy seam, lockstep N-env runner with identity-grouped day batches over `oracle.backend`, exact d4h0 opening handoff and stateless E economic history, strict-schema trajectory NPZ + JSON sidecar with PPO-ready slots, seed streams/provenance, executor factory seam, and an official-vs-fast parity seam. Evidence: 47 passed + 1 skipped; two deterministic tiny-random-E fast games = 52 transitions each, byte-equal rerun, [0,0] tie — plumbing only. Stage B (PPO updates) is NOT started; serious training stays blocked on issue #7 executor selection. Details: `research/RL_SELFPLAY_V0.md`. The issue-#8 gates stand: real-checkpoint parity once `artifacts/local/bc-v1-E/best.pt` is copied from Kaggle, then the (unmeasured) 8-device TPU benchmark per `research/JAX_TPU_V5_RUN.md` when hardware exists.
