...
- Stage 7 issue #12C opportunistic watering is mechanically correct and remains disabled by default: optional candidates are isolated from maintenance/productive watering, priority and starvation behavior, Stage 4 `WATER:` dependencies, hiring/debt metrics, and traceability all pass focused tests. The unbounded-distance concern is strategic tuning, not a proven defect; no enabling, clamp, or behavior patch is justified without the documented fixed 24-game false-vs-true A/B. Detail: `research/EXECUTOR_V07_STAGE7_ISSUE12C_OPTIONAL_WATER_AUDIT.md`.
