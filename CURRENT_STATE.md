...
## Snapshot
- Stage 1 / issue #13 packaging is implemented in `tools/build_submission.py`, `tools/verify_submission.py`, and the tracked `tools/submission_main.py` template. The builder emits a deterministic 50-member archive with the six verified local runtime packages, root `main.py`/`best.pt`, opening-book data, and no native extension/cache junk. The exact archive was built at SHA-256 `4ccfcf25d30465661c912626a5d029210897ec5855c3dc2b55db2cdfd1a7d6cf`; fresh-extract verification remains PARTIAL because the sanitized child environment has no `kaggle_environments`. The verifier already fails before the game when `fast_env` or `fast_env.market` is omitted, and production strict=False remains unchanged; no trajectory fingerprint is pinned until the official run is available.
...
- The real BC-E checkpoint used for validation was externally supplied read-only from `C:\Users\liuyi\VSCodeProjecs\Kaggriculture\Kaggriculture\artifacts\local\bc-v1-E\best.pt` (variant E, epoch 27, SHA-256 `F4B029D3E463ABA1DB0544377D0D616E3DE94AA6CC469D3446F018DDDD8F6BF2`). It is intentionally absent from this isolated worktree because local artifacts/checkpoints are ignored and remains uncommitted. Viewer #11 is closed and passive: schema/runner/snapshot/viewer/debug-trace CLI support is committed, four ignored real BC-E E_VS_E traces were generated and parsed, and ON/OFF action/result parity held for seeds 17 and 42; E_VS_E banks intentionally are not the PASS-panel result.
