...
def test_new_executor_controls_are_candidate_only():
    ...
    assert candidate.config.batch_reserved_supplies is True
    assert candidate.config.underfoot_queue_insertion is True
    ...
    assert opponent.config.batch_reserved_supplies is False
    assert opponent.config.underfoot_queue_insertion is False
    ...
    assert ignored.config.batch_reserved_supplies is False
    assert ignored.config.underfoot_queue_insertion is False
...
def test_starvation_visibility_cli_is_default_off_and_parseable():
    ...
    assert parser.parse_args(common).batch_reserved_supplies is False
    assert parser.parse_args(common).underfoot_queue_insertion is False
    assert parser.parse_args(
        common + ["--starvation-workload-visibility-repair"]
    ).starvation_workload_visibility_repair is True
    assert parser.parse_args(
        common + ["--batch-reserved-supplies", "--underfoot-queue-insertion"]
    ).batch_reserved_supplies is True
    assert parser.parse_args(
        common + ["--batch-reserved-supplies", "--underfoot-queue-insertion"]
    ).underfoot_queue_insertion is True
    assert parser.parse_args(
