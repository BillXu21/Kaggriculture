...
from rl_manager.stage25_adapter import _replaceable_today_arrays
...
def test_offline_and_live_replaceable_today_are_identical() -> None:
    ...
    offline_record = {"start": {"self": {"board": canonical}}}
    offline = _replaceable_today_arrays([offline_record], [16])
    ...
    assert offline.dtype == np.int16
    np.testing.assert_array_equal(offline, live_inputs["replaceable_today"])
