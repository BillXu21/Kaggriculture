...
def test_available_crop_slots_input_is_required_bounded_and_scalar_per_row():
    config = _config()
    params = init_stage25_params(config, seed=1072)
    omitted = _encoded(1)
    del omitted["available_crop_slots"]
    with pytest.raises(ValueError, match="available_crop_slots"):
        greedy_act(params, omitted, config)
    for bad in (np.zeros((1, 1), dtype=np.int16),
                np.full((1,), 101, dtype=np.int16),
                np.full((1,), -1, dtype=np.int16),
                np.full((1,), 1.5, dtype=np.float32)):
        inputs = _encoded(1)
...
def test_nonzero_available_slot_conditioning_changes_policy_representation():
    config = _config()
    params = dict(init_stage25_params(config, seed=1073))
    params["available_crop_slots_conditioning"] = jnp.full(
        (config.d_model,), 0.25, dtype=jnp.float32)
    zero = _encoded(1)
    occupied = dict(zero)
    occupied["available_crop_slots"] = np.asarray([50], dtype=np.int16)
    context = PhysicalContext(1, (25, 50, 75, 100), (0, 0, 0))
    before = greedy_act(params, zero, config, physical_contexts=(context,))
    after = greedy_act(params, occupied, config, physical_contexts=(context,))
    assert not np.array_equal(np.asarray(before["logits"]),
