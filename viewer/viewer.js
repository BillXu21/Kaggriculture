...
(function () {
  ...
  function render() {
    ...
    $("economy-panel").innerHTML = '<h2>Economy</h2><dl>' + kv("Cash", core.number(farm.money)) + kv("Hires today", farm.hires_today) + kv("Unlocked", (farm.unlocked_quadrants || []).join(", ") || "none") + kv("Town shops", ((state.town || {}).unlocked_shops || []).join(", ") || "none") + '</dl>';
    ...
    var actions = turn.joint_actions && turn.joint_actions[String(seat)], actionLines = actions ? ["canonical farmer: " + core.json(actions.farmer), "canonical hands: " + core.json(actions.hands), "canonical market: " + core.json(actions.market)] : []; if (executor && executor.actions) { actionLines.push("executor farmer: " + core.json(executor.actions.farmer)); actionLines.push("executor hands: " + core.json(executor.actions.hands)); } if (executor && executor.market) actionLines.push("submitted market: " + core.json(executor.market.submitted)); $("actions-panel").innerHTML = '<h2>Actions & movement</h2>' + list(actionLines, "No action data recorded for this turn.");
  }
