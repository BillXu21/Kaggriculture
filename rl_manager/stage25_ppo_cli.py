...
def _format_report(record: Mapping[str, Any]) -> str:
    ...
    profile_lines = [
        ...
        "provider prepare detail",
        line("prelude", f"{float(summed.get('stage25_provider_prelude_seconds', 0.0)):.4f} s"),
        line("step resolve", f"{float(summed.get('stage25_provider_step_resolution_seconds', 0.0)):.4f} s"),
        line("prev exec validation", f"{float(summed.get('stage25_provider_previous_execution_validation_seconds', 0.0)):.4f} s"),
        line("encode live inputs", f"{float(summed.get('stage25_provider_encode_live_inputs_seconds', 0.0)):.4f} s"),
        line("crop count", f"{float(summed.get('stage25_provider_crop_count_seconds', 0.0)):.4f} s"),
        line("unplaced animals", f"{float(summed.get('stage25_provider_unplaced_animals_seconds', 0.0)):.4f} s"),
        line("canonical board", f"{float(summed.get('stage25_provider_canonical_board_seconds', 0.0)):.4f} s"),
        line("physical context", f"{float(summed.get('stage25_provider_physical_context_seconds', 0.0)):.4f} s"),
        line("input freeze copy", f"{float(summed.get('stage25_provider_input_freeze_copy_seconds', 0.0)):.4f} s"),
        line("support payload", f"{float(summed.get('stage25_provider_support_payload_seconds', 0.0)):.4f} s"),
        line("row token", f"{float(summed.get('stage25_provider_row_token_seconds', 0.0)):.4f} s"),
        line("context build", f"{float(summed.get('stage25_provider_context_object_build_seconds', 0.0)):.4f} s"),
        line("accept input copy", f"{float(summed.get('stage25_provider_accept_input_copy_seconds', 0.0)):.4f} s"),
        line("bootstrap prepare", f"{float(summed.get('stage25_provider_bootstrap_seconds', 0.0)):.4f} s"),
        line("accept", f"{float(summed.get('stage25_provider_accept_seconds', 0.0)):.4f} s"),
        line("trajectory record", f"{float(summed.get('stage25_trajectory_record_seconds', 0.0)):.4f} s"),
        line("manager batch build", f"{float(summed.get('stage25_manager_batch_build_seconds', 0.0)):.4f} s"),
        line("daily utilization", f"{float(summed.get('stage25_daily_utilization_seconds', 0.0)):.4f} s"),
        line("identity check", f"{float(summed.get('stage25_identity_check_seconds', 0.0)):.4f} s"),
        "environment detail",
        line("native batch.step", f"{float(summed.get('fast_batch_step_seconds', 0.0)):.4f} s"),
        line("  action encode", f"{float(summed.get('fast_action_encode_seconds', 0.0)):.4f} s"),
        line("  native step", f"{float(summed.get('fast_native_step_seconds', 0.0)):.4f} s"),
        line("  observation decode", f"{float(summed.get('fast_observation_decode_seconds', 0.0)):.4f} s"),
        ...
        line("fast subphase sum", f"{float(derived.get('fast_subphase_sum_seconds', 0.0)):.4f} s"),
        line("fast step unattributed", f"{float(derived.get('fast_step_unattributed_seconds', 0.0)):.4f} s"),
        line("encode fraction", f"{float(derived.get('fast_action_encode_fraction', 0.0)):.4f}"),
        line("native fraction", f"{float(derived.get('fast_native_step_fraction', 0.0)):.4f}"),
        line("decode fraction", f"{float(derived.get('fast_observation_decode_fraction', 0.0)):.4f}"),
        line("provider subphase sum", f"{float(derived.get('provider_prepare_subphase_sum_seconds', 0.0)):.4f} s"),
        line("provider unattributed", f"{float(derived.get('provider_prepare_unattributed_seconds', 0.0)):.4f} s"),
        line("env share of worker wall", f"{float(derived.get('environment_share_of_worker_wall', 0.0)):.4f}"),
        line("batch.step share of wall", f"{float(derived.get('fast_batch_step_share_of_worker_wall', 0.0)):.4f}"),
        line("provider share of wall", f"{float(derived.get('provider_prepare_share_of_worker_wall', 0.0)):.4f}"),
        line("freeze bytes", f"{int(derived.get('provider_input_freeze_total_bytes', 0))} B"),
        line("accept copy bytes", f"{int(derived.get('provider_accept_copy_total_bytes', 0))} B"),
        "parent dispatch",
        ...
        line("  encode us/turn", f"{float(normalized.get('microseconds_per_active_env_turn_fast_action_encode', 0.0)):.2f}"),
        line("  native us/turn", f"{float(normalized.get('microseconds_per_active_env_turn_fast_native_step', 0.0)):.2f}"),
        line("  decode us/turn", f"{float(normalized.get('microseconds_per_active_env_turn_fast_observation_decode', 0.0)):.2f}"),
        ...
        line("  encode ms/step", f"{float(normalized.get('milliseconds_per_batch_step_fast_action_encode', 0.0)):.4f}"),
        line("  native ms/step", f"{float(normalized.get('milliseconds_per_batch_step_fast_native_step', 0.0)):.4f}"),
        line("  decode ms/step", f"{float(normalized.get('milliseconds_per_batch_step_fast_observation_decode', 0.0)):.4f}"),
        line("ms / manager row", f"{float(normalized.get('milliseconds_per_manager_row', 0.0)):.4f}"),
        line("  encode live ms/row", f"{float(normalized.get('milliseconds_per_manager_row_provider_encode_live_inputs', 0.0)):.4f}"),
        line("  canonical board ms/row", f"{float(normalized.get('milliseconds_per_manager_row_provider_canonical_board', 0.0)):.4f}"),
        line("  physical ctx ms/row", f"{float(normalized.get('milliseconds_per_manager_row_provider_physical_context', 0.0)):.4f}"),
        line("  freeze ms/row", f"{float(normalized.get('milliseconds_per_manager_row_provider_input_freeze', 0.0)):.4f}"),
        line("  support ms/row", f"{float(normalized.get('milliseconds_per_manager_row_provider_support_payload', 0.0)):.4f}"),
        line("  freeze KB/row", f"{float(normalized.get('kilobytes_per_manager_row_provider_input_freeze', 0.0)):.2f}"),
    ]
    return report + "\n" + "\n".join(profile_lines)
